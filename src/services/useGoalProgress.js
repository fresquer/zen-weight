...
export function useGoalProgress() {
  ...
  const stepsCompleted = computed(() => {
    if (
      !currentWeight.value ||
      !settings.value.goal_segments ||
      !settings.value.starting_weight ||
      !settings.value.target_weight
    )
      return 0
    const stepSize =
      (settings.value.starting_weight - settings.value.target_weight) / settings.value.goal_segments
  ...
  const stepCompletionPercentage = computed(() => {
    ...
    const stepStartWeight =
      ...
      stepsCompleted.value *
        ((settings.value.starting_weight - settings.value.target_weight) /
          settings.value.goal_segments)
    const stepSize =
      (settings.value.starting_weight - settings.value.target_weight) / settings.value.goal_segments
